...
[![Build Status](https://travis-ci.org/ckaznocha/SublimeLinter-contrib-write-good.svg?branch=master)](https://travis-ci.org/ckaznocha/SublimeLinter-contrib-write-good)

This linter plugin for [SublimeLinter][docs] provides an interface to [write-good](https://github.com/btford/write-good). It will be used with files that have English prose.
...
1. Install `write-good` by typing the following in a terminal:
   ```
...
1. When the plugin list appears, type `write-good`. Among the entries you should see `SublimeLinter-contrib-write-good`. If that entry is not highlighted, use the keyboard or mouse to select it.
